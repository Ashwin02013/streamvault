...
// GET DASHBOARD STATS — must be defined BEFORE /:id to avoid route conflict
...
// GET SINGLE USER — keep /:id AFTER all named routes
...
router.put('/:id/plan', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT cognito_id, username FROM users WHERE id = $1', [id]
    )
