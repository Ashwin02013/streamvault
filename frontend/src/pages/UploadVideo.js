      ...
      <div className="upload-card">
        ...
        <form onSubmit={handleSubmit} className="upload-form">
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
              <button type="button" onClick={() => setThumbnailMode('auto')}
                style={{ padding: '8px 16px', borderRadius: '6px', border: thumbnailMode === 'auto' ? '2px solid #e50914' : '1px solid #444', background: thumbnailMode === 'auto' ? '#e50914' : 'transparent', color: '#fff', cursor: 'pointer' }}>
                🤖 Auto
              </button>
              <button type="button" onClick={() => setThumbnailMode('capture')}
                style={{ padding: '8px 16px', borderRadius: '6px', border: thumbnailMode === 'capture' ? '2px solid #e50914' : '1px solid #444', background: thumbnailMode === 'capture' ? '#e50914' : 'transparent', color: '#fff', cursor: 'pointer' }}>
                🎞️ Capture Frame
              </button>
              <button type="button" onClick={() => setThumbnailMode('upload')}
                style={{ padding: '8px 16px', borderRadius: '6px', border: thumbnailMode === 'upload' ? '2px solid #e50914' : '1px solid #444', background: thumbnailMode === 'upload' ? '#e50914' : 'transparent', color: '#fff', cursor: 'pointer' }}>
                🖼️ Upload Image
              </button>
            </div>
            ...
            {thumbnailMode === 'capture' && videoFile && (
              <div>
                <p style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '8px' }}>Drag to select frame time:</p>
                <input type="range" min="0" max={videoDuration} value={captureTime}
                  onChange={handleCaptureTimeChange} style={{ width: '100%', marginBottom: '10px' }} />
            ...
            {thumbnailMode === 'capture' && !videoFile && (
              <p style={{ color: '#e50914', fontSize: '0.9rem' }}>Please select a video file first.</p>
            )}
            ...
            <input id="thumbInput" type="file" accept="image/*" style={{ display: 'none' }} onChange={handleThumbnailUpload} />
          ...
          <div className="form-group">
            ...
            <textarea name="description" placeholder="Enter video description"
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input type="text" name="duration" placeholder="Auto-detected on upload"
